...
```{r girls-baptized-over-the-years}
ggplot(data = arbuthnot, aes(x = year, y = girls)) + 
  geom_line()+labs(title = "Number of girls baptized over the years", x="Years", y="Girls Baptized")
```

#### **Trend:**

Well the general overall trend of the girls baptized over the years is on the rise with few exceptional dips. The girls baptized experiences a rise in the first few years until 1640 where it experiences a huge drop of approximately 2500 girls in 10 years and dwells like that till 1660 where the numbers start to pick up again and carries the uptrend into the early 1700s
...
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_line()+labs(title = "Proportion of boys over the years", x="Years", y="Boy's Ratio")
```

We can see that the proportion of boys baptized over the years has been oscillating around the mean value of almost .518.
...
The years included in the data  set start from 1940 to all the way till 2002.


1.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?

There are two ways to check the magnitude of both data set. Firstly, we can use "comparedf()" function from **"arsenal"** which will give us an overview of both data sets. Secondly, we can simply check the summary of both data sets separately and compare there mins and maxs etc. Both ways has been carried out in the solution below but the "comparedf()" method has been commented out.
```{r}
#install.packages("arsenal")
#library(arsenal)
data('arbuthnot', package='openintro')
#summary(comparedf(arbuthnot,present)) #comparison of data frames
glimpse(arbuthnot)
glimpse(present)
```

Using glimpse function we can see that Arbuthnot's data has more observations compared to present but if we are talking about the magnitude of number present inside the observations than present data has higher magnitude 
...
ggplot(data = present, aes(x = year, y = boys_ratio)) + 
  geom_line()+labs(title = "Proportion of boys over the years for present", x="Years", y="Boy's Ratio")+theme_classic()
```

Well the graph for present data set clearly depicts that the proportion of boys is on an oscillation downward trend while in Arbuthnot's data portrays an oscillating proportion around a mean value as explained in exercise 3. Moreover, the number boys born/baptized are still more than girl since the proportion of boys is still over .5.
...
head(present %>%
  arrange(desc(total)))
```
1961 is year with more total births according to present data set. The results of all top six years with highest number of births has been presented above.
