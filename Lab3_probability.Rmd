...
**Answer:** The streak of 1 means 1 hit and 1 miss. The streak of length 0 means no hits with a miss
...
**Answer:** The most common or typical streak length according to the bar chart above is 0 with the longest streak of 4 baskets. The graph shows skewness to the right. As shown below:
...
**Answer:** In the unfair simulation, 17 times the coin flipped out to be head
```{r sim-unfair}
...
**Answer:** We will have to add a **prob** argument to our **sample** function and increase the **size to 133**
...
According to the bar chart the distribution is skewed right with typical streak length of 0 and 4 baskets as the longest streak.
...
**Answer:** If I ran the simulation of independent shooter a second time I might not get the exact identical results but it will have similar kind of distribution. The streak length might get affected and since the sample size is not that large we might witness some variance but the overall distribution will be similar.
...
**Answer:** The simulated distribution that we have created is very similar to Kobe Bryant's distribution. Kobe's highest streak is 4 baskets and so does simulated distribution over here. I did run the simulation a couple of times and got different longest streak i.e. 5,6,7 e.t.c., but  the overall distribution of bar chart was very similar. Over here we can clearly see that our simulated model is independent and is closely followed by Kobe's, So we can conclude that Kobe was not a **Hot Hand**.
